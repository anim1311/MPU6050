...
bool MPU6050::begin(uint8_t i2c_address, TwoWire *wire,
                             int32_t sensor_id) {
  if (i2c_dev) {
  ...
  BusIO_Register chip_id =
      BusIO_Register(i2c_dev, MPU6050_WHO_AM_I, 1);
...
bool MPU6050::_init(int32_t sensor_id) {
  ...
  BusIO_Register power_mgmt_1 =
      BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_1, 1);
...
void MPU6050::reset(void) {
  BusIO_Register power_mgmt_1 =
      BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_1, 1);
  BusIO_Register sig_path_reset =
      BusIO_Register(i2c_dev, MPU6050_SIGNAL_PATH_RESET, 1);
  BusIO_RegisterBits device_reset =
      BusIO_RegisterBits(&power_mgmt_1, 1, 7);
...
uint8_t MPU6050::getSampleRateDivisor(void) {
  BusIO_Register sample_rate_div =
      BusIO_Register(i2c_dev, MPU6050_SMPLRT_DIV, 1);
...
mpu6050_accel_range_t MPU6050::getAccelerometerRange(void) {
  BusIO_Register accel_config =
      BusIO_Register(i2c_dev, MPU6050_ACCEL_CONFIG, 1);
  BusIO_RegisterBits accel_range =
      BusIO_RegisterBits(&accel_config, 2, 3);
...
mpu6050_gyro_range_t MPU6050::getGyroRange(void) {
  BusIO_Register gyro_config =
      BusIO_Register(i2c_dev, MPU6050_GYRO_CONFIG, 1);
  BusIO_RegisterBits gyro_range =
      BusIO_RegisterBits(&gyro_config, 2, 3);
...
void MPU6050::setClock(mpu6050_clock_select_t new_clock) {
  BusIO_Register pwr_mgmt =
      BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_1, 1);

  BusIO_RegisterBits clock_select =
      BusIO_RegisterBits(&pwr_mgmt, 3, 0);
...
mpu6050_fsync_out_t MPU6050::getFsyncSampleOutput(void) {
  BusIO_Register config =
      BusIO_Register(i2c_dev, MPU6050_CONFIG, 1);
  BusIO_RegisterBits fsync_out =
      BusIO_RegisterBits(&config, 3, 3);
...
mpu6050_bandwidth_t MPU6050::getFilterBandwidth(void) {
  ...
  BusIO_RegisterBits filter_config =
      BusIO_RegisterBits(&config, 3, 0);
...
void MPU6050::setInterruptPinPolarity(bool active_low) {
  BusIO_Register int_pin_config =
      BusIO_Register(i2c_dev, MPU6050_INT_PIN_CONFIG, 1);
  BusIO_RegisterBits int_level =
      BusIO_RegisterBits(&int_pin_config, 1, 7);
...
void MPU6050::setInterruptPinLatch(bool held) {
  BusIO_Register int_pin_config =
      BusIO_Register(i2c_dev, MPU6050_INT_PIN_CONFIG, 1);
  BusIO_RegisterBits int_latch =
      BusIO_RegisterBits(&int_pin_config, 1, 5);
...
void MPU6050::setMotionInterrupt(bool active) {
  BusIO_Register int_enable =
      BusIO_Register(i2c_dev, MPU6050_INT_ENABLE, 1);
  BusIO_RegisterBits int_motion =
      BusIO_RegisterBits(&int_enable, 1, 6);
...
bool MPU6050::getMotionInterruptStatus(void) {
  BusIO_Register status =
      BusIO_Register(i2c_dev, MPU6050_INT_STATUS, 1);

  BusIO_RegisterBits motion =
      BusIO_RegisterBits(&status, 1, 6);
...
void MPU6050::setMotionDetectionThreshold(uint8_t thr) {
  BusIO_Register threshold =
      BusIO_Register(i2c_dev, MPU6050_MOT_THR, 1);
  threshold.write(thr);
}
...
void MPU6050::setMotionDetectionDuration(uint8_t dur) {
  BusIO_Register duration =
      BusIO_Register(i2c_dev, MPU6050_MOT_DUR, 1);
  duration.write(dur);
}
...
void MPU6050::setI2CBypass(bool bypass) {
  BusIO_Register int_pin_config =
      BusIO_Register(i2c_dev, MPU6050_INT_PIN_CONFIG, 1);
  BusIO_RegisterBits i2c_bypass =
      BusIO_RegisterBits(&int_pin_config, 1, 1);

  BusIO_Register user_ctrl =
      BusIO_Register(i2c_dev, MPU6050_USER_CTRL, 1);
  BusIO_RegisterBits i2c_master_enable =
      BusIO_RegisterBits(&user_ctrl, 1, 5);
...
bool MPU6050::enableSleep(bool enable) {
  ...
  BusIO_RegisterBits sleep =
      BusIO_RegisterBits(&pwr_mgmt, 1, 6);
...
bool MPU6050::enableCycle(bool enable) {
  ...
  BusIO_RegisterBits cycle =
      BusIO_RegisterBits(&pwr_mgmt, 1, 5);
...
mpu6050_cycle_rate_t MPU6050::getCycleRate(void) {
  BusIO_Register pwr_mgmt_2 =
      BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_2, 1);

  BusIO_RegisterBits cycle_rate =
      BusIO_RegisterBits(&pwr_mgmt_2, 2, 6);
 ...
 */
/**************************************************************************/
bool MPU6050::setGyroStandby(bool xAxisStandby, bool yAxisStandby,
                                      bool zAxisStandby) {
  BusIO_Register pwr_mgmt_2 =
      BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_2, 1);

  BusIO_RegisterBits gyro_stdby =
      BusIO_RegisterBits(&pwr_mgmt_2, 3, 0);
 ...
 */
/**************************************************************************/
bool MPU6050::setAccelerometerStandby(bool xAxisStandby,
                                               bool yAxisStandby,
                                               bool zAxisStandby) {
  BusIO_Register pwr_mgmt_2 =
      BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_2, 1);

  BusIO_RegisterBits accel_stdby =
      BusIO_RegisterBits(&pwr_mgmt_2, 3, 3);
...
bool MPU6050::setTemperatureStandby(bool enable) {
  ...
  BusIO_RegisterBits temp_stdby =
      BusIO_RegisterBits(&pwr_mgmt, 1, 3);
...
void MPU6050::_read(void) {
  // get raw readings
  BusIO_Register data_reg =
      BusIO_Register(i2c_dev, MPU6050_ACCEL_OUT, 14);
...
/**************************************************************************/
/*!
    @brief  Gets the most recent sensor event,  Unified Sensor format
    @param  accel
            Pointer to an  Unified sensor_event_t object to be filled
...
/*!
  @brief  Gets an  Unified Sensor object for the temp sensor component
...
/*!
    @brief  Gets an  Unified Sensor object for the accelerometer
    sensor component
...
/*!
    @brief  Gets an  Unified Sensor object for the gyro sensor component
    @return Sensor pointer to gyro sensor
 */
Sensor *MPU6050::getGyroSensor(void) { return gyro_sensor; }
